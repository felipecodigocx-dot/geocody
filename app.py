...
def processar_excel(filepath):
    ...
    try:
        ...
        return dados, tem_quantidade
        
    except Exception as e:
        raise Exception(f"Erro ao processar Excel: {str(e)}")

def criar_mapa_tradicional(dados_coordenadas):
    """
...
def criar_mapa_calor(dados_coordenadas):
    """
...
def criar_mapa_circulos(dados_coordenadas):
    """
...
def criar_mapa_coordenadas(dados_coordenadas, tipo_mapa='tradicional', tem_quantidade=False):
    """
    Cria mapa baseado no tipo selecionado
    """
    try:
        if not dados_coordenadas:
            raise ValueError("Nenhum dado de coordenada fornecido")
        
        # Adicionar estatísticas ao mapa
        if tipo_mapa == 'tradicional':
            mapa = criar_mapa_tradicional(dados_coordenadas)
        elif tipo_mapa == 'calor' and tem_quantidade:
            mapa = criar_mapa_calor(dados_coordenadas)
        elif tipo_mapa == 'circulos' and tem_quantidade:
            mapa = criar_mapa_circulos(dados_coordenadas)
        else:
            # Fallback para tradicional
            mapa = criar_mapa_tradicional(dados_coordenadas)
        
        # Calcular centro para estatísticas
        lats = [ponto['latitude'] for ponto in dados_coordenadas]
        lons = [ponto['longitude'] for ponto in dados_coordenadas]
        centro_lat = sum(lats) / len(lats)
        centro_lon = sum(lons) / len(lons)
        ...
        <h4>Estatísticas</h4>
        <p><strong>Total de pontos:</strong> {len(dados_coordenadas)}</p>
        <p><strong>Tipo:</strong> {tipo_mapa.title()}</p>
        """
        
        if tem_quantidade:
            quantidades = [ponto.get('quantidade', 0) for ponto in dados_coordenadas]
...
def upload_file():
    ...
    try:
        ...
        # Processar Excel
        dados_coordenadas, tem_quantidade = processar_excel(filepath)
        ...
        # Criar mapa
        mapa = criar_mapa_coordenadas(dados_coordenadas, tipo_mapa, tem_quantidade)
        
        # Salvar mapa HTML
        map_filename = f"mapa_{tipo_mapa}_{uuid.uuid4()}.html"
        ...
        response_data = {
            'success': True,
            'message': f'Mapa {tipo_mapa} gerado com sucesso! {len(dados_coordenadas)} pontos processados.',
            'map_url': f'/map/{map_filename}',
            'download_url': f'/download/{map_filename}',
            'points_count': len(dados_coordenadas),
            'map_type': tipo_mapa,
...
@app.route('/exemplo')
def exemplo_excel():
    """Gera um arquivo Excel de exemplo com coordenadas e quantidades para demonstrar todas as funcionalidades"""
    try:
